...
describe(flatJoinAsync.name, () => {
  describe('fluent...flatJoinAsync', () => {
    it('should flat join the specified path', async () => {
      ...
      expect(result[0][tail]).to.be.eq(source0);
      expect(result[0][head]).to.be.eq(
        source0.field1.field2[0].field3.field1[0],
      );
      expect(result[1][tail]).to.be.eq(source0);
      expect(result[1][head]).to.be.eq(
        source0.field1.field2[0].field3.field1[1],
      );
      expect(result[2][tail]).to.be.eq(source0);
      expect(result[2][head]).to.be.eq(
        source0.field1.field2[0].field3.field1[2],
      );
      expect(result[3][tail]).to.be.eq(source0);
      expect(result[3][head]).to.be.eq(source0.field1.field2[1].field3.field1);
      expect(result[4][tail]).to.be.eq(source1);
      expect(result[4][head]).to.be.eq(
        source1.field1[0].field2.field3.field1[0],
      );
      expect(result[5][tail]).to.be.eq(source1);
