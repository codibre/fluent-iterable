...
function* interval(init: any, final: any) {
  for (let i = init; i <= final; i++) {
    yield i;
  }
}
function interval2(init: any, final: any) {
  const items: any[] = [];
  for (let i = init; i <= final; i++) {
    items.push(i);
  }
...
function* map1(it: any) {
  for (const x of it) {
    yield x * MULTIPLIER1;
  }
}
function* map2(it: any) {
  for (const x of it) {
    yield x * MULTIPLIER2;
  }
}
function* filter1(it: any) {
  for (const x of it) {
    if (x % QUOTIENT === 0) {
      yield x;
    }
  ...
  it('fluent should win', async () => {
    benchmarkSuite
      ...
      .add('native builtin iterator', () => {
        (interval(1, ITEMS) as any)
          .map((x: any) => x * MULTIPLIER1)
          .map((x: any) => x * MULTIPLIER2)
          .map((x: any) => x / MULTIPLIER2)
          .map((x: any) => x * MULTIPLIER2)
          .map((x: any) => x / MULTIPLIER2)
          .map((x: any) => x * MULTIPLIER2)
          .filter((x: any) => x % QUOTIENT === 0)
          .map((x: any) => x / MULTIPLIER2)
          .map((x: any) => x * MULTIPLIER2)
          .map((x: any) => x / MULTIPLIER2)
          .map((x: any) => x * MULTIPLIER2)
          ...
          .map((x: any) => x / MULTIPLIER2)
          .map((x: any) => x * MULTIPLIER2)
          .map((x: any) => interval2(x, x + FLAT_FACTOR))
          .take(TAKE)
          .forEach((x: any) => x.join(','));
      })
      ...
      .on('complete', function (this: any) {
        console.log(log);
        expect(this.filter('fastest').map('name')).toInclude('fluent');
      })
      .run();
  });
});
