  ...
  function executionSuite(repetition: number, total: number, items: number) {
    return () => {
      it(`fluent should be faster than rxjs with ${total} times and ${items} items`, async () => {
        ...
        console.log(`Total start time Rxjs..: ${avgStartRxjs}`);
        console.log(`Total exec time Rxjs..: ${avgExecRxjs}`);
        console.log(`Total start time Fluent: ${avgStartFluent}`);
        console.log(`Total exec time Fluent: ${avgExecFluent}`);
        console.log(`Proportion start..: ${avgStartFluent / avgStartRxjs}`);
        console.log(`Proportion exec..: ${avgExecFluent / avgExecRxjs}`);
        console.log('---------------------------');
        expect(avgExecRxjs > avgExecFluent).to.be.true;
      });
    };
  }
